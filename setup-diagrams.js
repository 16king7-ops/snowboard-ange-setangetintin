...
(() => {
  ...
  SETUP_DIAGRAMS.add({
    ...
    cant: {
      cap: "フットベッドは「外側（板の中心から遠い側）」が少し高い2.5°の傾き。L/R を入れ替えると高い側が内側に来るので、実物で確かめる。",
      h: 278,
      draw: (K) => {
        const Y = 146;
        ...
        b += K.text(90, 172, "かかと側から見た図（左が前足・右が後足）", { cls: "t-s t-dim", a: "middle" });
        // 「外側」がどちら側かと、左右の入れ違い
        b += K.text(10, 190, "外側＝板の中心から遠い側", { cls: "t-s" });
        b += K.text(10, 204, "（前足はノーズ側・後足はテール側）", { cls: "t-s" });
        b += K.text(10, 222, "L/R は足の左右。レギュラーなら", { cls: "t-s" });
        b += K.text(10, 236, "L＝前足（ノーズ側）・R＝後足（テール側）", { cls: "t-s" });
        b += K.text(10, 256, "つま先側はトゥランプで上がっているので、", { cls: "t-s" });
        b += K.text(10, 270, "高い側は土踏まずのあたりで比べる", { cls: "t-s" });
        return b;
      },
    },
