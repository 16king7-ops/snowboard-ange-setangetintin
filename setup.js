...
function renderMount() {
  ...
  const groups = [
    ...
    ["ブーツに合わせる", [
      ...
      ["フットベッドの左右", `L＝左足・R＝右足。${S.rd.regular ? "レギュラーなので L がノーズ側（前足）、R がテール側（後足）" : "グーフィーなので R がノーズ側（前足）、L がテール側（後足）"}。入れたら、傾きの高い側が外側（板の中心から遠い側）に来ているか見る。逆なら左右が入れ替わっている。` + tb, "cant"],
